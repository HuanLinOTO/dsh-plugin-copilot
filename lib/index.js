			...
			interaction: {
				...
				prompt: (prompt) => {
					if (isEnterpriseDomainPrompt(prompt)) {
						const domain = ((typeof deps.enterpriseDomain === "function" ? deps.enterpriseDomain() : deps.enterpriseDomain) ?? "").trim();
...
const Config = z.object({ enterpriseDomain: z.string().default("").description("GitHub Enterprise domain (e.g. company.ghe.com); blank serves github.com").volatile() });
...
function apply(ctx, config = {}) {
	...
	const piAiSection = () => {
		...
		const raw = settings.describe().find((row) => row.ns === PI_AI_NS)?.value;
