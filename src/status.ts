...
export const COPILOT_SCOPE = 'llm-pi-ai'
/** Settings namespace whose `providers` dict carries the Copilot profile. */
export const COPILOT_SETTINGS_NS = 'llm-pi-ai'
 ...
 */
export const COPILOT_RECORD_KEY: CredentialKey = credentialKey(COPILOT_SCOPE, COPILOT_PROVIDER)

/** The joined record address, as the authorization flow registry reports it. */
export function recordAddress(key: CredentialKey): string {
 ...
 */
export function findCopilotFlow(entries: readonly AuthorizationEntry[]): AuthorizationEntry | undefined {
  return entries.find((entry) => {
    const address = recordAddress(entry.key)
    return address === `${COPILOT_SCOPE}/${COPILOT_PROVIDER}`
  })
}
 ...
 */
export function grantModelIds(record: CredentialRecord | undefined): readonly string[] | undefined {
  if (record?.kind !== 'grant') return undefined
  const models = (record.payload as { availableModelIds?: unknown } | null)?.availableModelIds
  if (!Array.isArray(models) || !models.every(id => typeof id === 'string')) return undefined
  return models
}
 ...
 */
export async function joinStatus(sources: StatusSources): Promise<CopilotStatus> {
  ...
  const section = sources.settingsSection()
  const providers = section?.providers
  const profileActivated = typeof providers === 'object' && providers !== null
    && Object.hasOwn(providers as Record<string, unknown>, COPILOT_PROVIDER)
  return {
    flowAvailable: flow !== undefined,
    loggedIn,
    profileActivated,
    inFlight: flow?.inFlight ?? false,
    models: loggedIn ? await sources.models() : undefined,
  }
}
